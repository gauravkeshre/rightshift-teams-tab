...
const RIGHTSHIFT_DB_DIRECTORY = "db"; // Directory where LowDB files are stored
const rightshiftDirectory = join(dirname(dirname(fileURLToPath(import.meta.url))), RIGHTSHIFT_DB_DIRECTORY);
...
export class dbService {
    ...
    async getTable(tableName, primaryKey) {
        ...
        if (!tables[tableKey]) {
            ...
            const file = join(rightshiftDirectory, `${tableName}.json`);
