...
const EMAIL_DOMAIN = "rightshift.com"; // For making fake email addresses for employees

const db = new dbService();     // Singleton service for our rightshift DB
...
export async function getEmployee(employeeId) {
    ...
    result.photo = employee.Photo.substring(104); // Trim RightShift-specific junk
...
export async function getEmployeeByLastName(lastName) {
    ...
    // This works only because lastname is unique in the RightShift sample
    ...
    const result = {
        ...
        photo: employee.Photo.substring(104), // Trim RightShift-specific junk
...
export async function getOrder(orderId) {
    ...
    result.employeeEmail = `${employee.LastName.toLowerCase()}@RightShifttraders.com`;
    ...
    result.details = details.map((item) => {
        ...
        return {
            ...
            categoryPicture: category.Picture.substring(104), // Remove RightShift-specific junk
...
export async function getCategories() {
    ...
    const result = categories.data.map(category => ({
        ...
        picture: category.Picture.substring(104), // Remove RightShift-specific junk
    }));
...
export async function getCategory(categoryId) {
    ...
    result.picture = category.Picture.substring(104); // Remove RightShift-specific junk
